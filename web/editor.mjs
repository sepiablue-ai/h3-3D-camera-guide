...
let state=structuredClone(DEFAULT), config={width:576,height:1024,frames:124,fps:24};
...
$('stop').onclick=()=>seek(0);
function resize(){const a=$('world').getBoundingClientRect(),b=$('preview').getBoundingClientRect();wr.setSize(a.width,a.height,false);editorCam.aspect=a.width/a.height;editorCam.updateProjectionMatrix();pr.setSize(b.width,b.height,false);}
...
window.addEventListener('message',e=>{if(e.source!==parent||e.origin!==location.origin||e.data?.type!=='h3-camera-load')return;try{
  ...
  config={...config,...e.data.config};trajectory();keysUI();seek(Math.min(time,(config.frames-1)/config.fps));
}catch(err){$('status').textContent=`状態を読めません: ${err.message}`;}});
window.addEventListener('error',e=>$('status').textContent=e.message);
let last=performance.now();
function animate(now){const dt=Math.min((now-last)/1000,.1);last=now;if(playing){time=Math.min(time+dt,(config.frames-1)/config.fps);pose=sample(state,time);if(time>=(config.frames-1)/config.fps)playing=false;sync();}wr.render(world,editorCam);
